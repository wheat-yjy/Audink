package com.kotlinproject.wooooo.audink.view
...
import com.kotlinproject.wooooo.audink.tool.PagePaintAdapter
import com.kotlinproject.wooooo.audink.tool.PagePainter
import com.kotlinproject.wooooo.audink.tool.PageTextCutter
import com.kotlinproject.wooooo.audink.utils.sp2px
import java.io.File
...
class PageView : View {
    ...
    val currentPage  get() =  pageTextCutter.getCurrentPage()
    val totalPage get() = pageTextCutter.getTotalPages()


    constructor(context: Context) : super(context)
    constructor(context: Context, attrs: AttributeSet) : super(context, attrs)

    private val pageAdapter by lazy { PagePaintAdapter(context, sp2px(context, 18f)) }
    private val pageTextCutter by lazy { PageTextCutter(pageAdapter) }
    private val pagePainter by lazy { PagePainter(pageAdapter) }

    fun loadText(text: String) {
        Log.i(TAG, "inputText: $text")
        pageTextCutter.load(text)
//        pageTextCutter.load(testText)
    }

    fun loadFile(file: File) {
        val str = file.readText()
        pageTextCutter.load(str)
    }

    fun nextPage() {
        pageTextCutter.nextPage()
        invalidate()
    }

    fun prePage() {
        pageTextCutter.prePage()
        invalidate()
    }

    override fun onDraw(canvas: Canvas?) {
        super.onDraw(canvas)
        val article = pageTextCutter.getCuttedText()
        canvas?.let { pagePainter.drawText(it, article) }
    }
